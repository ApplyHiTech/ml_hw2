...
class Problem(object):
    ...
    def move_pacman(self, state, action):
        next_tile = None
        ...
        if state[next_tile] == 10 or state[next_tile] == 11:
            if self.state[next_tile] == 11:
                self.accumulated_reward += checker.PILL_CONSTANT
            state[next_tile] = 66
        ...
        # ghosts and poison
        if state[next_tile] in checker.LOSS_INDEXES:
            state[next_tile] = 88
            state[(current_tile_x, current_tile_y)] = 10
    ...
    def result(self, state, action):
        ...
        self.actions(state)."""
        self.change_state_after_action(state,action)
    ...
    def goal_test(self, state):
        ...
        return self.finished_the_game(state)
        # if isinstance(self.goal, list):
        #     return is_in(state, self.goal)
        # else:
        #     return state == self.goal

    def finished_the_game(self, state):
        finished = True
        breaking_flag = False
...
def best_first_graph_search(problem, f):
    ...
    while frontier:
        node = frontier.pop()
        if node.state.pMoves == ['U', 'D', 'L', 'L', 'U', 'U', 'R', 'R']:
            print("po")
        if problem.goal_test(node.state):
            return node
        explored.add(node.state)
        for child in node.expand(problem):
            # MEIR
            if child.state.pMoves == ['U', 'D', 'L', 'L', 'U', 'U', 'R', 'R']:
                print("po")
            if child.state not in explored and child not in frontier:
                frontier.append(child)
